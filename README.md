...
- **自动对抗系统代理**：把 Adobe 域名加入系统代理绕过列表，并写入 Clash Verge 自己的配置（代理重启也不丢），应用无法借代理绕过防火墙
- **hosts 域名兜底**：把 23 个 Adobe 授权/正版校验域名解析到 0.0.0.0，双重保险
...
| 脚本 | 作用 | 需要管理员 |
|---|---|---|
| `block-adobe-network.ps1` | 发现 Adobe 安装 → 为所有 .exe 建出站+入站拦截规则 → 必要时启用防火墙 → 备份授权缓存 → 代理绕过（含 Clash Verge 配置持久化）→ hosts 域名兜底 → 停止后台辅助进程 | 是 |
...
5. **绕代理**：把 Adobe 域名加入系统代理绕过列表，防止应用借代理绕过防火墙规则；同时写入 Clash Verge 自身配置，代理重启不再丢失
6. **hosts 兜底**：把 Adobe 授权/正版校验域名（23 个）解析到 0.0.0.0
7. **验证**：确认防火墙 Enabled、规则生效、无遗漏程序
...
- 防火墙规则只拦 Adobe 程序本身，不影响浏览器访问 adobe.com；hosts 兜底仅拦 Adobe 授权/正版校验域名
- hosts 改动对全系统生效（其他程序访问这些授权域名也会被拦），属预期行为；恢复联网时 `unblock-adobe-network.ps1` 会一并清理
...
- **Defeats system proxies**: adds Adobe domains to the proxy bypass list (and persists it inside Clash Verge's own config so it survives proxy restarts), so apps cannot tunnel around the firewall rules via Clash/mihomo
- **hosts fallback**: resolves 23 Adobe licensing/genuine-check domains to 0.0.0.0 as a second line of defense
...
|---|---|---|
| `block-adobe-network.ps1` | Discover Adobe installs → create out+in block rules for every .exe → enable firewall if needed → back up license cache → proxy bypass (incl. Clash Verge persistence) → hosts fallback → stop background helpers | Yes |
...
5. **Proxy bypass**: adds Adobe domains to the system proxy bypass list so apps connect directly and get caught by the firewall rules; also persists it inside Clash Verge's own config so proxy restarts cannot wipe it.
6. **hosts fallback**: resolves 23 Adobe licensing/genuine-check domains to 0.0.0.0.
7. **Verify**: confirms the firewall is on, rules are active, and nothing is missed.
...
- Firewall rules block only Adobe processes; your browser can still reach adobe.com. The hosts fallback only covers Adobe licensing/genuine-check domains.
- The hosts changes apply system-wide (any program touching those license domains is blocked too) — expected behavior; `unblock-adobe-network.ps1` cleans them up when restoring.
